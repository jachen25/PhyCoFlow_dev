...
NonlinearPoissonDataset = BASELINE_HELPERS.NonlinearPoissonDataset
ElasticityDataset = BASELINE_HELPERS.ElasticityDataset
AirfoilCGridDataset = BASELINE_HELPERS.AirfoilCGridDataset
AirfoilInterpDataset = BASELINE_HELPERS.AirfoilInterpDataset
AirfoilWakeLESDataset = BASELINE_HELPERS.AirfoilWakeLESDataset
CarCFDDataset = BASELINE_HELPERS.CarCFDDataset
...
backup_existing_artifact = BASELINE_HELPERS.backup_existing_artifact
...
SUPPORTED_DATASETS = {"turbulent_combustion", "elasticity", "airfoil",
                      "airfoil_interp", "airfoil_interp_5f", "airfoil_wake",
                      "car_cfd", "poisson", "active_emulsion"}
# Datasets with regular-grid topology required by S3GM and LatentFM (UNet) adapters.
# The airfoil_interp* variants sit on a fixed uniform grid, so they qualify.
GRID_DATASETS = {"turbulent_combustion", "elasticity", "airfoil",
                 "airfoil_interp", "airfoil_interp_5f"}
...
def safe_torch_load(path: Path, map_location: str | torch.device = "cpu") -> dict:
    ...
    try:
        ...
        # torch>=2.6 defaults weights_only=True, which rejects checkpoints that
        # pickle callables (e.g. GeoFNO stores its activation fn). These are
        # our own locally trained checkpoints, so the full unpickle is trusted.
...
def validate_and_normalize_config(cfg: dict) -> dict:
    ...
    # Derive dataset-aware defaults so a config that only sets `dataset_name`
    # still lands its artifacts under the correct per-dataset subfolders.
    _ds_name = str(shared["data"]["dataset_name"]).strip().lower()
    _default_data_path = {
    ...
    }.get(_ds_name, "Dataset/Merged_CH4COTU1P.h5")
    shared.setdefault("paths", {})
    shared["paths"].setdefault("data_path", _default_data_path)
    shared["paths"].setdefault("save_root", f"Save_TrainedModel/{_ds_name}")
    shared["paths"].setdefault("config_backup_root", f"Save_config/{_ds_name}/{baseline_model}")
    ...
    # Coarse-grid / pooled-observation super-resolution conditioning (the FFM
    # N16-N19 protocol). Defaults preserve the legacy random-sensor behavior.
    shared["conditioning"].setdefault("sensor_layout", "independent")
    shared["conditioning"].setdefault("obs_grid_stride_list", None)
    shared["conditioning"].setdefault("obs_grid_pool", False)
    shared["conditioning"].setdefault("obs_grid_pool_physical", False)
    _strides = shared["conditioning"]["obs_grid_stride_list"]
    _any_strided = bool(_strides) and any(int(s) > 1 for s in _strides)
    if shared["conditioning"]["obs_grid_pool"] and not _any_strided:
        raise ValueError(
            "obs_grid_pool=true requires obs_grid_stride_list with at least "
            "one stride > 1.")
    if shared["conditioning"]["obs_grid_pool_physical"] and not shared["conditioning"]["obs_grid_pool"]:
        raise ValueError(
    ...
    dataset_name = str(shared["data"]["dataset_name"]).strip().lower()
    if dataset_name not in SUPPORTED_DATASETS:
        raise ValueError(
            f"Unknown dataset_name={dataset_name!r}. "
            f"Expected one of {sorted(SUPPORTED_DATASETS)}."
        )
    shared["data"]["dataset_name"] = dataset_name

    # S3GM and LatentFM operate on a regular grid (UNet); reject irregular meshes.
    if baseline_model in {"s3gm", "latent_fm"} and dataset_name not in GRID_DATASETS:
        raise ValueError(
            f"baseline_model={baseline_model!r} requires a grid-compatible dataset; "
            f"got {dataset_name!r}. Supported here: {sorted(GRID_DATASETS)}."
        )
    return cfg
...
def build_dataset(cfg: dict, split: str, stats_path: Path):
    """Construct the train/val Dataset for the configured `dataset_name`.

    The user supplies dataset-specific kwargs under `shared.data.<key>`; sane
    defaults match the adapter expectations for combustion / elasticity /
    airfoil / car_cfd / poisson.
    """
    shared = cfg["shared"]
    data_path = ensure_absolute(shared["paths"]["data_path"])
    data_cfg = shared["data"]
    dataset_name = str(data_cfg["dataset_name"]).strip().lower()
    train_ratio = float(data_cfg["train_ratio"])
    seed = int(shared["seed"])

    if dataset_name == "turbulent_combustion":
        return TurbulentCombustionH5Dataset(
            str(data_path), split=split, train_ratio=train_ratio,
            seed=seed, time_stride=int(data_cfg["time_stride"]),
            stats_path=str(stats_path),
        )
    if dataset_name == "airfoil_wake":
        # Mid-span plane of the UMich turbulent airfoil wake LES. Field
        # reconstruction (no mask): fixed 2D point cloud, sparse velocity
        # sensors -> full instantaneous turbulent field. Same project H5
        # schema as combustion (see preprocess_airfoil_wake_les.py).
        return AirfoilWakeLESDataset(
            str(data_path), split=split, train_ratio=train_ratio,
            seed=seed, time_stride=int(data_cfg.get("time_stride", 1)),
            stats_path=str(stats_path),
        )
    if dataset_name == "elasticity":
        return ElasticityDataset(
            str(data_path), split=split, train_ratio=train_ratio,
            seed=seed, stats_path=str(stats_path),
        )
    if dataset_name == "airfoil":
        return AirfoilCGridDataset(
            data_dir=str(data_path), split=split, train_ratio=train_ratio,
            seed=seed, stats_path=str(stats_path),
            select_fields=data_cfg.get("select_fields"),
            sensor_surface_offset_min=int(data_cfg.get("sensor_surface_offset_min", 1)),
            sensor_surface_offset_max=int(data_cfg.get("sensor_surface_offset_max", 3)),
        )
    if dataset_name in ("airfoil_interp", "airfoil_interp_5f"):
        # Uniform-grid airfoil variant: the shape is NOT given via coordinates
        # (fixed grid) and instead appears as a trailing `mask` channel that the
        # model must infer. interp_subdir selects the 1-field or 5-field release.
        interp_subdir = "naca_interp_5f" if dataset_name == "airfoil_interp_5f" else "naca_interp"
        # Sensor placement: 'near_surface' (default, hugs the body) or 'ellipse'
        # (fixed contour, no shape leakage through sensor locations). Ellipse
        # geometry is configured in the model's [0,1]^2 frame.
        ellipse_center = data_cfg.get("ellipse_center", (0.5, 0.5))
        ellipse_semi_axes = data_cfg.get("ellipse_semi_axes", (0.30, 0.12))
        return AirfoilInterpDataset(
            ...
            sensor_surface_offset_min=int(data_cfg.get("sensor_surface_offset_min", 1)),
            sensor_surface_offset_max=int(data_cfg.get("sensor_surface_offset_max", 3)),
            interp_subdir=data_cfg.get("interp_subdir", interp_subdir),
            sensor_placement=str(data_cfg.get("sensor_placement", "near_surface")),
            ellipse_center=tuple(ellipse_center),
            ellipse_semi_axes=tuple(ellipse_semi_axes),
            ellipse_ring_halfwidth=float(data_cfg.get("ellipse_ring_halfwidth", 0.08)),
        )
    if dataset_name == "car_cfd":
        return CarCFDDataset(
            data_dir=str(data_path), split=split, train_ratio=train_ratio,
            seed=seed, n_points=int(data_cfg.get("car_n_points", 8192)),
            stats_path=str(stats_path),
        )
    if dataset_name == "poisson":
        return NonlinearPoissonDataset(
            str(data_path), split=split, train_ratio=0.7, seed=seed,
            n_points=int(data_cfg.get("poisson_n_points", 6000)),
            n_bound=int(data_cfg.get("poisson_n_bound", 1024)),
            stats_path=str(stats_path),
        )
    if dataset_name == "active_emulsion":
        # Reuse the FFM tree's dataset so the baseline sees byte-identical
        # data: cell-level splits.json protocols, asinh flow normalization,
        # and the physical-pooling contract that build_sparse_condition's
        # pool_value_transform consumes. Stats live in the dataset's own
        # protocol/fields/transform-keyed cache under data_root (shared with
        # the FFM runs), not the per-run stats_path.
        try:
            from helpers import ActiveEmulsionDataset
        except ImportError:
            from .helpers import ActiveEmulsionDataset
        pool_physical = bool(
            shared["conditioning"].get("obs_grid_pool_physical", False))
        kwargs = dict(
            protocol=str(data_cfg.get("ae_protocol", "interp")),
            splits_path=data_cfg.get("ae_splits_path"),
            fields=tuple(data_cfg.get("ae_fields", ("phi", "vx", "vy"))),
            seed=seed,
            flow_transform=str(data_cfg.get("ae_flow_transform", "asinh")),
            pool_observations_physical=pool_physical,
        )
        if split == "train":
            # Train-only frame thinning and exact torus augmentation, mirroring
            # the FFM trainer; validation stays full and unaugmented.
            kwargs.update(
                frame_downsample=bool(data_cfg.get("ae_frame_downsample", False)),
                frame_tau=float(data_cfg.get("ae_frame_tau", 0.02)),
                frame_min=int(data_cfg.get("ae_frame_min", 4)),
                augment=str(data_cfg.get("ae_augment", "none")),
            )
        return ActiveEmulsionDataset(str(data_path), split=split, **kwargs)
    raise ValueError(f"Unsupported dataset_name={dataset_name!r}")


def resolve_sr_condition_kwargs(cfg: dict, dataset) -> dict:
    """Extra build_sparse_condition kwargs for coarse-grid / pooled sensors.

    SINGLE SOURCE for the baseline-side super-resolution conditioning: the
    epoch loops and the deterministic visualizer both resolve their stride /
    pooling arguments here, mirroring the FFM trainer's prepare_rf_batch
    wiring so a baseline sees the exact observation operator the FFM run
    used. Returns {} when the config requests plain random sensors, keeping
    every pre-SR call site's behavior unchanged.
    """
    cond = cfg["shared"]["conditioning"]
    strides = cond.get("obs_grid_stride_list")
    if not strides or not any(int(s) > 1 for s in strides):
        return {}
    base = dataset
    ...
    obs_grid_pool = bool(cond.get("obs_grid_pool", False))
    pool_value_transform = (
        BASELINE_HELPERS.resolve_pooled_value_transform(base)
        if obs_grid_pool else None)
    if bool(cond.get("obs_grid_pool_physical", False)) and pool_value_transform is None:
        raise ValueError(
            "obs_grid_pool_physical=true, but the dataset does not provide "
            "the physical pooling transform contract "
            "(pool_observations_physical + denormalize/normalize_field).")
    return dict(
        sensor_layout=str(cond.get("sensor_layout", "independent")),
        Ny=int(grid_shape[0]),
        Nx=int(grid_shape[1]),
        obs_grid_strides=list(strides),
...
def run_epoch_s3gm(bundle: BaselineBundle, loader: DataLoader, training: bool, epoch: int) -> float:
    ...
    # bf16 autocast on CUDA only; no GradScaler needed for bf16's fp32 range.
    use_amp = bool(bundle.components.get("use_amp", False)) and bundle.device.type == "cuda"
...
def run_epoch_sit(bundle: BaselineBundle, loader: DataLoader, training: bool, epoch: int) -> float:
    ...
    accum_steps = max(1, int(resolve_stage_config(bundle.config)["training"].get("grad_accum_steps", 1)))

    net.train(training)
    total_loss = 0.0
    count = 0
    micro = 0                # gradient-accumulation micro-step counter
    accum_loss_sum = 0.0     # sum of per-micro-batch losses in the current window
    pbar = tqdm(loader, desc=f"SiT Epoch {epoch:04d} [{'Train' if training else 'Eval'}]", leave=False)

    def _sit_apply_step(window_loss: float) -> None:
        # Gradients for the full accumulation window are in place (each micro-batch
        # backward was scaled by 1/accum_steps, so this is the mean gradient of an
        # effective batch = accum_steps * physical batch). Apply the same spike-skip
        # + EMA logic as the unaccumulated path, at the window level.
        grad_norm = float(nn.utils.clip_grad_norm_(all_params_fn(), max_norm=0.5))
        skip = not (math.isfinite(window_loss) and math.isfinite(grad_norm))
        ema_loss = spike_state.get("ema_loss")
        ema_grad = spike_state.get("ema_grad")
        if not skip and ema_loss is not None and window_loss > 5.0 * ema_loss:
            skip = True
        ...
        if skip:
            ...
            spike_state["ema_loss"] = (
                window_loss if ema_loss is None else beta * ema_loss + (1.0 - beta) * window_loss
            )
    ...
    for batch in pbar:
        ...
        if training and optimizer is not None:
            ...
            micro += 1
            accum_loss_sum += current_loss
            if micro >= accum_steps:
                _sit_apply_step(accum_loss_sum / micro)
                micro = 0
                accum_loss_sum = 0.0
    ...
    # Flush a partial accumulation window (epoch length not divisible by accum_steps).
    if training and optimizer is not None and micro > 0:
        _sit_apply_step(accum_loss_sum / micro)
...
def run_epoch_senseiver(bundle: BaselineBundle, loader: DataLoader, training: bool, epoch: int) -> float:
    ...
    accum_steps = max(1, int(stage_cfg["training"].get("grad_accum_steps", 1)))
    sr_kwargs = resolve_sr_condition_kwargs(bundle.config, loader.dataset)
...
def dps_sample(
    ...
    alpha_obs=1.0,
    eps=1e-3,
    corrector_clamp_max: float = 1.0,
    soft_replace: bool = False,
    guidance_mode: str = "resnorm",
    gamma: float = 1e-2,
    std_y: float = 0.0,
    trace_out=None,
):
    ...
    for i in tqdm(range(N_steps), desc="S3GM sampling", leave=False):
        ...
        for _ in range(n_corrector_steps):
            ...
            step_size = torch.clamp(step_size, max=corrector_clamp_max)
        ...
        with torch.enable_grad():
            ...
            # Tweedie denoiser estimate of x0. The sigma**2 factor is the correct
            # VE-SDE Tweedie formula (x0_hat = x + sigma^2 * score), NOT the source
            # of the blow-up.
            x0_hat = std_e ** 2 * score + x_inp
            residual = (obs_5d - x0_hat) * mask_5d
            # Standard DPS guidance: descend the gradient of the residual *norm*,
            # i.e. normalize the data-term gradient by ||residual||. grad(sum r^2)
            # is 2 * J^T r, which scales with ||r|| and explodes at high noise;
            # dividing by ||r|| makes the step O(alpha_obs * ||J||) and bounded
            # (the denoiser Jacobian J is a contraction). The previous unnormalized
            # sum-of-squares step (clamped only at +-1e8) caused the intermittent
            # sampler divergence to L2 ~ hundreds.
            sq_loss = torch.sum(residual ** 2)
            if torch.isfinite(sq_loss) and sq_loss > 0:
                if guidance_mode == "var_anneal":
                    # Reference S3GM data term: divide the squared residual by
                    # var = std_y^2 + gamma*sigma^2 (noise-level annealed), so the
                    # guidance is weak at high sigma (unreliable x0_hat) and ramps
                    # up as sigma -> 0. var is a scalar => grad(sq_loss/var) =
                    # grad(sq_loss)/var.
                    var = std_y ** 2 + gamma * float(sigma_cur) ** 2
                    raw = torch.autograd.grad(sq_loss / var, x_inp)[0]
                    raw = torch.nan_to_num(raw, nan=0.0, posinf=0.0, neginf=0.0)
                    dx = alpha_obs * raw
                else:  # "resnorm" (default): normalize the gradient by ||residual||
                    raw = torch.autograd.grad(sq_loss, x_inp)[0]      # 2 * J^T residual
                    raw = torch.nan_to_num(raw, nan=0.0, posinf=0.0, neginf=0.0)
                    residual_norm = torch.sqrt(sq_loss + 1e-12)
                    dx = alpha_obs * raw / residual_norm
                dx = torch.clamp(dx, min=-1e3, max=1e3)              # safety net
            else:
        ...
        if soft_replace:
            # Variance-aware soft data-consistency: pull observed pixels toward
            # the observation diffused to the current noise level, with weight
            # ramping 0 -> ~1 as the schedule anneals. Mirrors the reference
            # 1/var ramp without an unbounded gradient, so it cannot diverge.
            w = float(1.0 - t_cur)
            noised_obs = obs_5d + sigma_cur * torch.randn_like(obs_5d)
            x = torch.where(mask_5d > 0.5, (1.0 - w) * x + w * noised_obs, x)
            x = torch.nan_to_num(x, nan=0.0, posinf=0.0, neginf=0.0)

        if trace_out is not None:
            # Per-step diagnostics: state std and Tweedie x0_hat std vs sigma.
            # If x0_hat stays ~data-std*8 at low sigma, the score net is
            # non-restoring there (off-manifold attractor confirmed).
            trace_out.append((int(i), float(sigma_cur), float(x.std()),
                              float(x0_hat.detach().std())))

    return x.squeeze(1)


def _baseline_rel_l2(u_true: np.ndarray, u_pred: np.ndarray) -> float:
    # Mirrors helpers_baseline._normalized_l2 (kept local: helpers_baseline is
    # only imported lazily in this module).
...
def _baseline_payload(
    ...
    """Raw-array payload for offline evaluation; same dict shape as
    helpers_baseline.visualize_reconstruction(return_payload=True)."""
    return {
        "coords_xy": coords_xy,
...
def visualize_reconstruction_s3gm(
    ...
    # Reseed AFTER the sensor draw, never before: the sensors stay a pure
    # function of the caller's seed (so they are IDENTICAL across methods and
    # across ensemble members), while the sampling noise varies per member.
    if sampler_seed is not None:
        torch.manual_seed(int(sampler_seed))
        if torch.cuda.is_available():
            torch.cuda.manual_seed_all(int(sampler_seed))
    ...
    for c, name in enumerate(field_names):
        if not write_field_plots:
            metrics[name] = _baseline_rel_l2(truth_phys[:, c], recon_phys[:, c])
            continue
        sensor_coords = None
    ...
    if return_payload:
        return metrics, _baseline_payload(
            coords_xy, truth_phys, recon_phys, obs_indices_cpu,
            obs_field_ids_cpu, field_names, snapshot_index, cond_fields, n_obs,
        )
    return metrics
...
def visualize_reconstruction_latentfm(
    ...
    # Reseed AFTER the sensor draw (both branches above), never before: the
    # sensors stay a pure function of the caller's seed (so they are IDENTICAL
    # across methods and across ensemble members), while the sampling noise in
    # model.sample() below varies per member. This is what makes a K-draw
    # ensemble mean well-defined.
    if sampler_seed is not None:
        torch.manual_seed(int(sampler_seed))
        if torch.cuda.is_available():
            torch.cuda.manual_seed_all(int(sampler_seed))
    ...
    if return_payload:
        # Normalized (metrics, payload) interface for offline evaluation;
        # extra metrics (if requested) travel inside `metrics`.
        if extra_metrics:
...
def visualize_reconstruction_sit(
    ...
    # Reseed AFTER the sensor draw, never before: the sensors stay a pure
    # function of the caller's seed (so they are IDENTICAL across methods and
    # across ensemble members), while the sampling noise below varies per
    # member. This is what makes a K-draw ensemble mean well-defined.
    if sampler_seed is not None:
        torch.manual_seed(int(sampler_seed))
        if torch.cuda.is_available():
            torch.cuda.manual_seed_all(int(sampler_seed))
    ...
    for idx, name in enumerate(field_names):
        if not write_field_plots:
            metrics[name] = _baseline_rel_l2(truth_phys[:, idx], recon_phys[:, idx])
            continue
        sensor_coords = None
...
def _attach_senconsis_outputs(
    *,
    ...
    """Attach SenConsis (sensor-consistency) outputs for the deterministic path.

    Computes relative-L2 agreement between the reconstruction and the sparse
    observations at the sensor entries (in normalized space, matching the model
    objective) and merges the scalar metrics into ``metrics`` in place. When
    ``save_obs_consistency_plots`` is set, also writes the SenConsis/ JSON and
    the parity/residual plots. Mirrors the inline block in the generative
    visualizer (helpers.py); the deterministic path has no obs-consistency
    smoothing mode, so the smooth-mask plot is intentionally omitted.
    """
    # Visualization-only deps; imported lazily (same convention as
    # helpers.py's `from physics_metrics import ...`).
    try:
        from helpers import (
    ...
    if save_obs_consistency_plots:
        senconsis_dir = os.path.join(str(save_dir), "SenConsis")
...
def visualize_reconstruction_deterministic(
    ...
    obs_coords, obs_values, obs_mask, obs_indices, obs_field_ids = build_sparse_condition(
        ...
        n_obs_max=n_obs,
        valid_mask=valid_mask,
        **resolve_sr_condition_kwargs(bundle.config, dataset),
    )
    ...
    if hasattr(dataset, "asinh_scale") and hasattr(dataset, "denormalize"):
        # Active-emulsion flow channels are asinh-compressed before
        # standardization; mean/std alone does not return physical units.
        recon_phys = dataset.denormalize(recon)
        truth_phys = dataset.denormalize(truth)
    else:
    ...
    for field_idx, field_name in enumerate(field_names):
        if not write_field_plots:
            metrics[field_name] = _baseline_rel_l2(truth_np[:, field_idx], recon_np[:, field_idx])
            continue
        sensor_coords = None
    ...
    if compute_physics:
        from physics_metrics import compute_physics_metrics
        phys = compute_physics_metrics(dataset, truth_np, recon_np, snapshot_index)
        if phys:
            metrics["physics"] = phys
    ...
    if return_payload:
        return metrics, _baseline_payload(
            coords_xy, truth_np, recon_np, obs_indices_cpu,
            obs_field_ids_cpu, field_names, snapshot_index, cond_fields, n_obs,
        )
    return metrics
...
class BaseBaselineAdapter(abc.ABC):
    ...
    @abstractmethod
    def visualize(self, bundle: BaselineBundle, dataset, save_dir: Path, epoch: int, snapshot_index: int, n_steps: Optional[int] = None, compute_physics: bool = False, write_field_plots: bool = True, return_payload: bool = False):
        raise NotImplementedError
...
class S3GMAdapter(BaseBaselineAdapter):
    ...
    def visualize(self, bundle: BaselineBundle, dataset, save_dir: Path, epoch: int, snapshot_index: int, n_steps: Optional[int] = None, compute_physics: bool = False, write_field_plots: bool = True, return_payload: bool = False, sampler_seed: Optional[int] = None):
        stage_cfg = resolve_stage_config(bundle.config)
        shared_cond = bundle.config["shared"]["conditioning"]
        ...
        return visualize_reconstruction_s3gm(
            ...
            write_field_plots=write_field_plots,
...
class LatentFMAdapter(BaseBaselineAdapter):
    ...
    def visualize(self, bundle: BaselineBundle, dataset, save_dir: Path, epoch: int, snapshot_index: int, n_steps: Optional[int] = None, compute_physics: bool = False, write_field_plots: bool = True, return_payload: bool = False, sampler_seed: Optional[int] = None):
        ...
        if bundle.training_stage == 1:
            if return_payload:
                raise ValueError("return_payload is only supported for latent_fm stage 2 (the AE stage is not a reconstruction method).")
            if sampler_seed is not None:
                raise ValueError("sampler_seed (ensemble sampling) is only supported for latent_fm stage 2; the stage-1 AE reconstruction is deterministic.")
        ...
        result = visualize_reconstruction_latentfm(
            model=bundle.model,
            dataset=dataset,
            Num_x=int(bundle.config["shared"]["data"]["num_x"]),
            Num_y=int(bundle.config["shared"]["data"]["num_y"]),
...
class SenseiverAdapter(BaseBaselineAdapter):
    ...
    def visualize(self, bundle: BaselineBundle, dataset, save_dir: Path, epoch: int, snapshot_index: int, n_steps: Optional[int] = None, save_obs_consistency_plots: bool = False, compute_physics: bool = False, write_field_plots: bool = True, return_payload: bool = False):
...
class GeoFNOAdapter(BaseBaselineAdapter):
    ...
    def load_checkpoint(self, bundle: BaselineBundle, checkpoint: dict) -> None:
        ...
        if isinstance(state, dict):
            # geofno checkpoints serialized the state_dict's ``_metadata``
            # attribute as a plain key; load_state_dict rejects it.
